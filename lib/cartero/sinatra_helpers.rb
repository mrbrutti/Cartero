  ...
  def process_info(params,request)
    ...
    if (params[:key] && params[:key] != "" && params[:key] != nil ) || cookies["session_info"] != nil
      ...
      if settings.metasploit
        ...
        if (params[:password] || params[:pwd] || params[:secret] || params[params.keys.select {|x| x =~ /pass|pwd|secret/i }[0]]) != nil &&
...
module CrawlerBlock
  ...
  def block_and_redirect(url)
    before {
      redirect url, 301, "redirecting..." if request.user_agent.match(CRAWLERS)
    }
  end
end
end
